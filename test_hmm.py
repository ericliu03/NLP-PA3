...
from hmm import HMM
from hmm import UnsupervisedHMM as UHMM
...
class IceCreamHMM(TestCase):
    def setUp(self):
        ...
        self.hmm.train_ice_cream([],
                                 initial_probabilities=[.8, .2],        # P(Hot), P(Cold)
                                 transition_probabilities=[[.7, .3],    # P(Hot|Hot), P(Cold|Hot)
                                                           [.4, .6]],   # P(Hot|Cold), P(Cold|Cold)
...
class TagHMM(TestCase):
    ...
    @skip("here compute likelihood")
    def test_likelihood(self):
        print "likelihood: ", self.hmm.likelihood(self.test[0])
